...
ALTER TABLE user_security_holdings ADD CONSTRAINT chk_quantity_positive CHECK (quantity > 0);
ALTER TABLE user_security_holdings ADD CONSTRAINT chk_avg_price_positive CHECK (average_price > 0);
ALTER TABLE fixed_deposits ADD CONSTRAINT chk_fd_principal_positive CHECK (principal_amount > 0);
ALTER TABLE fixed_deposits ADD CONSTRAINT chk_fd_interest_rate_positive CHECK (interest_rate > 0);
ALTER TABLE fixed_deposits ADD CONSTRAINT chk_fd_tenure_positive CHECK (tenure_months > 0);
ALTER TABLE recurring_deposits ADD CONSTRAINT chk_rd_installment_positive CHECK (monthly_installment > 0);
ALTER TABLE recurring_deposits ADD CONSTRAINT chk_rd_interest_rate_positive CHECK (interest_rate > 0);
ALTER TABLE recurring_deposits ADD CONSTRAINT chk_rd_tenure_positive CHECK (tenure_months > 0);
ALTER TABLE user_assets ADD CONSTRAINT chk_asset_quantity_positive CHECK (quantity > 0);
ALTER TABLE user_assets ADD CONSTRAINT chk_asset_purchase_price_positive CHECK (purchase_price > 0);
ALTER TABLE asset_valuations ADD CONSTRAINT chk_valuation_amount_positive CHECK (valuation_amount > 0);
ALTER TABLE asset_transactions ADD CONSTRAINT chk_transaction_quantity_positive CHECK (quantity > 0);
ALTER TABLE asset_transactions ADD CONSTRAINT chk_transaction_price_positive CHECK (price_per_unit > 0);
ALTER TABLE portfolio_goals ADD CONSTRAINT chk_goal_target_amount_positive CHECK (target_amount > 0);
ALTER TABLE asset_allocation_targets ADD CONSTRAINT chk_allocation_percentage_valid CHECK (target_percentage >= 0 AND target_percentage <= 100);